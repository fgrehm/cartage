...
// ListResult is the action-specific data returned in Response.Data for a list.
...
// ParseListResult decodes daemon response data into a typed ListResult.
func ParseListResult(data any) (ListResult, error) {
	b, err := json.Marshal(data)
	if err != nil {
		return ListResult{}, fmt.Errorf("failed to encode response data: %w", err)
	}
	var r ListResult
	if err := json.Unmarshal(b, &r); err != nil {
		return ListResult{}, fmt.Errorf("failed to decode secret list result: %w", err)
	}
	return r, nil
}
...
func (h *Handler) handleList() (*protocol.Response, error) {
	refs, err := listSecrets()
	if err != nil {
